package citycircle.com.getPhoto;
...
//			startActivity(intent);
    ...
    protected void onDestroy() {
        super.onDestroy();
        AlbumActivity.this.unregisterReceiver(broadcastReceiver);
    }
        ...
        if (!(dataList.size() == 0)) {
            for (int i = 0; i < dataList.size(); i++) {
               for (int j=0;j<dataList.size();j++){
                   int id1 = Integer.parseInt(dataList.get(i).getImageId());
                   int id2 = Integer.parseInt(dataList.get(j).getImageId());
                   if (id2<id1){
                       ImageItem imageItem = dataList.get(i);
                       dataList.set(i, dataList.get(j));
                       dataList.set(j, imageItem);
                   }
               }
            }
        }
